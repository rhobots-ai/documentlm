...
from ktem.storage.s3_storage import S3Storage
...
class DocumentRetrievalPipeline(BaseFileIndexRetriever):
    ...
    def run(
        ...
        **kwargs,
        ...
        """
        # Convert doc_ids to list if it's a string (single UUID)
        ...
        with Session(engine) as session:
            ...
            chunk_ids = [str(r[0].target_id) for r in results.all()]

        # do first round top_k extension
        retrieval_kwargs["do_extend"] = True
        retrieval_kwargs["thumbnail_count"] = 50
...
class IndexPipeline(BaseComponent):
    ...
    def get_id_if_exists(self, file_path: str | Path) -> Optional[str]:
        ...
        if self.private:
            cond: tuple = (
                self.Source.title == file_name,
                self.Source.created_by_id == self.user_id,
            )
        else:
            cond = (self.Source.title == file_name,)
    ...
    def store_file(self, file_path: Path) -> str:
        ...
        # Check if S3 storage is enabled
            if getattr(flowsettings, "KH_USE_S3_STORAGE", False):
                s3_storage = S3Storage(prefix=self.user_id)
                file_path_str = s3_storage.upload_file(str(file_path), file_hash)  # Save the URL
                print("This is file URL:", file_path_str)
            else:
                # Local storage logic
                shutil.copy(file_path, self.FSPath / file_hash)
                file_path_str = str(self.FSPath / file_hash)
    ...
    def finish(self, file_id: str, file_path: str | Path) -> str:
        ...
        with Session(engine) as session:
            ...
            if doc_ids and token_func:
                ...
                item.data["tokens"] = sum([len(token_func(doc.text)) for doc in docs])

            # populate the note
            item.data["loader"] = self.get_from_path("loader").__class__.__name__
        ...
        with Session(engine) as session:
            ...
            for each in index:
                if each[0].relation_type == "vector":
                    vs_ids.append(str(each[0].target_id))
                elif each[0].relation_type == "document":
                    ds_ids.append(str(each[0].target_id))
    ...
    def stream(
        ...
        yield Document({'file_id': file_id}, channel="info")
...
class IndexDocumentPipeline(BaseFileIndexIndexing):
    ...
    def save_temp_file(file):
        """
        Create a temporary file in the system's temp directory and save the content
        from S3 to it.

        Args:
            file: An object containing file information with a 'file' attribute that
                 contains the S3 key path.

        Returns:
            str: The path to the created temporary file.
        """

        # Create a temporary file with a meaningful extension if possible
        if hasattr(file, 'title') and file.title:
    ...
    @staticmethod
    def _clean_temp_files(file_paths):
        ...
        for path in file_paths:
            try:
                if os.path.exists(path):
                    ...
                    logger.warning(f"FastAPI: Temporary file not found for cleanup: {path}")
            except Exception as e:
                logger.warning(f"FastAPI: Could not remove temp file {path}: {str(e)}")
    ...
    def stream(
        self, file_ids: str | Path | list[str | Path], reindex: bool = False, **kwargs
    ) -> Generator[
        ...
        for idx, file_id in enumerate(file_ids):
            ...
            try:
                ...
                if file.data_type != 'url':
                    self._clean_temp_files([file_path])
                errors.append(None)
                yield Document(
                    content={
                        "file_path": file_path,
    ...
    def delete_file(self, file_ids: list[str]):
        ...
        with Session(engine) as session:
            index = session.execute(
                select(self.Index).where(self.Index.source_id.in_(file_ids))
            ).all()
            vs_ids, ds_ids = [], []
            for each in index:
                if each[0].relation_type == "vector":
