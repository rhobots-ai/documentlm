...
class FileIndexPage(BasePage):
    ...
    def list_file(self, user_id, name_pattern=""):
        ...
        with Session(engine) as session:
            ...
            if self._index.config.get("private", False):
                statement = statement.where(Source.created_by_id == user_id)
            if name_pattern:
                statement = statement.where(Source.title.ilike(f"%{name_pattern}%"))
            results = [
                {
                    "id": each[0].id,
                    "name": each[0].title,
                    ...
                    "tokens": self.format_size_human_readable(
                        each[0].data.get("tokens", "-"), suffix=""
                    ),
                    "loader": each[0].data.get("loader", "-"),
                    "date_created": each[0].created_at.strftime("%Y-%m-%d %H:%M:%S"),
                }
    ...
    def list_group(self, user_id, file_list):
        ...
        with Session(engine) as session:
            ...
            if self._index.config.get("private", False):
                statement = statement.where(FileGroup.created_by_id == user_id)
...
class FileSelector(BasePage):
    ...
    def load_files(self, selected_files, user_id):
        ...
        with Session(engine) as session:
            ...
            if self._index.config.get("private", False):
                statement = statement.where(
                    self._index._resources["Source"].created_by_id == user_id
                )
            ...
            for result in results:
                ...
                options.append((result[0].title, result[0].id))
