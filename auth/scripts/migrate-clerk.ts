...
import { auth } from "../lib/auth.ts"; // import your auth instance
...
async function readCSVFile() {
  try {
    ...
    console.log(exportedUserCSV);
    return exportedUserCSV;
  } catch (error) {
...
const exportedUserCSV = await readCSVFile()
...
async function getClerkUsers(totalUsers: number) {
  ...
  for (let i = 0; i < totalUsers; i += 500) {
    const response = await fetch(`https://api.clerk.com/v1/users?offset=${i}&limit=${500}`, {
      headers: {
        'Authorization': `Bearer sk_test_zptXF2eXZmqe47q8oxcX1tEERT6ev2sknrN8bwgosj`
      }
    });
    if (!response.ok) {
...
function safeDateConversion(timestamp?: number): Date {
  ...
  if (year < 2000 || year > 2100) {
    console.warn(`Suspicious date year: ${year}, ${timestamp} falling back to current date`);
    return new Date();
  }
